...
class SubcategoryControllerTest {
    ...
    @Test
    fun `POST subcategories should return 201 with created subcategory`() {
        every { createSubcategoryUseCase.execute(any()) } returns subcategoryResponse

        assertThat(
            mockMvc.post().uri("/v1/subcategories")
                .contentType(MediaType.APPLICATION_JSON)
                .content("""{"name":"Fuel","categoryId":2}""")
        ).hasStatus(201)
    }

    @Test
    fun `POST subcategories should return 400 when body is malformed`() {
        assertThat(
            mockMvc.post().uri("/v1/subcategories")
                .contentType(MediaType.APPLICATION_JSON)
                .content("{}")
    ...
    @Test
    fun `POST subcategories should return 409 when subcategory already exists`() {
        every { createSubcategoryUseCase.execute(any()) } throws SubcategoryAlreadyExistsException("Fuel", 2L)

        val result = mockMvc.post().uri("/v1/subcategories")
            .contentType(MediaType.APPLICATION_JSON)
            .content("""{"name":"Fuel","categoryId":2}""")
    ...
    @Test
    fun `POST subcategories should return 404 when category not found`() {
        every { createSubcategoryUseCase.execute(any()) } throws CategoryNotFoundException(99L)

        assertThat(
            mockMvc.post().uri("/v1/subcategories")
                .contentType(MediaType.APPLICATION_JSON)
                .content("""{"name":"Fuel","categoryId":99}""")
        ).hasStatus(404)
    }

    @Test
    fun `GET subcategories should return 200 with full list`() {
        every { findSubcategoryUseCase.findAll(null, null) } returns listOf(subcategoryResponse)

        assertThat(mockMvc.get().uri("/v1/subcategories")).hasStatusOk()
    }

    @Test
    fun `GET subcategories should forward name filter`() {
        every { findSubcategoryUseCase.findAll("Fuel", null) } returns listOf(subcategoryResponse)

        assertThat(
            mockMvc.get().uri("/v1/subcategories").param("name", "Fuel")
        ).hasStatusOk()
    }

    @Test
    fun `GET subcategories should forward categoryId filter`() {
        every { findSubcategoryUseCase.findAll(null, 2L) } returns listOf(subcategoryResponse)

        assertThat(
            mockMvc.get().uri("/v1/subcategories").param("categoryId", "2")
        ).hasStatusOk()
    }

    @Test
    fun `GET subcategories by id should return 200 with subcategory`() {
        every { findSubcategoryUseCase.findById(5L) } returns subcategoryResponse

        assertThat(mockMvc.get().uri("/v1/subcategories/5")).hasStatusOk()
    }

    @Test
    fun `GET subcategories by id should return 404 when not found`() {
        every { findSubcategoryUseCase.findById(99L) } throws SubcategoryNotFoundException(99L)

        val result = mockMvc.get().uri("/v1/subcategories/99")
    ...
    @Test
    fun `PUT subcategories should return 200 with updated subcategory`() {
        every { updateSubcategoryUseCase.execute(5L, any()) } returns subcategoryResponse

        assertThat(
            mockMvc.put().uri("/v1/subcategories/5")
                .contentType(MediaType.APPLICATION_JSON)
                .content("""{"name":"Fuel","categoryId":2}""")
        ).hasStatusOk()
    }
    ...
    @Test
    fun `PUT subcategories should return 404 when not found`() {
        every { updateSubcategoryUseCase.execute(99L, any()) } throws SubcategoryNotFoundException(99L)

        assertThat(
            mockMvc.put().uri("/v1/subcategories/99")
                .contentType(MediaType.APPLICATION_JSON)
                .content("""{"name":"Fuel","categoryId":2}""")
        ).hasStatus(404)
    }
    ...
    @Test
    fun `PUT subcategories should return 409 when name conflict`() {
    ...
    @Test
    fun `DELETE subcategories should return 204 when deleted`() {
        every { deleteSubcategoryUseCase.execute(5L) } returns Unit

        assertThat(mockMvc.delete().uri("/v1/subcategories/5")).hasStatus(204)

        verify { deleteSubcategoryUseCase.execute(5L) }
    }

    @Test
    fun `DELETE subcategories should return 404 when not found`() {
        every { deleteSubcategoryUseCase.execute(99L) } throws SubcategoryNotFoundException(99L)

        assertThat(mockMvc.delete().uri("/v1/subcategories/99")).hasStatus(404)
    }
}
