...
interface CategoryJpaRepository : JpaRepository<CategoryEntity, Long> {
    ...
    @Query("SELECT c FROM CategoryEntity c LEFT JOIN FETCH c.subcategories WHERE LOWER(c.name) LIKE LOWER(CONCAT('%', :name, '%'))")
    fun findAllWithSubcategoriesByName(@Param("name") name: String): List<CategoryEntity>
